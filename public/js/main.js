...
function filterShops(shops, selectedArea, selectedGenre, searchShopName) {
  return shops.filter(function (shop) {
    var isAreaMatch = !selectedArea || shop.area_id === parseInt(selectedArea);
    var isGenreMatch = !selectedGenre || shop.genre_id === parseInt(selectedGenre);
    var isShopNameMatch = !searchShopName || shop.name.toLowerCase().includes(searchShopName.toLowerCase());
...
function updateSearchResults() {
  ...
  var searchShopName = searchInput.value;
  var filteredShops = filterShops(shops, selectedArea, selectedGenre, searchShopName);
...
function handleFavoriteButtonClick(event) {
  ...
  var favoriteButton = clickedElement.closest('.favorite-btn');
  if (!favoriteButton) {
    return;
  }
  var shopId = favoriteButton.dataset.shopId;
  if (isUserLoggedIn) {
    ...
    favoriteShop(shopId)["finally"](function () {
...
function favoriteShop(_x) {
  return _favoriteShop.apply(this, arguments);
}
function _favoriteShop() {
  _favoriteShop = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(shopId) {
    var favoriteButton, response, heartIcon;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          favoriteButton = document.querySelector(".favorite-btn[data-shop-id=\"".concat(shopId, "\"]"));
        ...
        case 5:
          ...
          heartIcon = favoriteButton.querySelector('.fa-heart');
...
function updateFavoriteButtons() {
  ...
  favoriteButtons.forEach(function (button) {
    ...
    var isFavorited = window.favorites.some(function (favorite) {
      return favorite.id === parseInt(shopId, 10);
    });
...
document.addEventListener('DOMContentLoaded', function () {
  var currentUrl = window.location.href;
  var isIndexPage = currentUrl.includes('/index') || currentUrl.endsWith('/');
  if (isIndexPage) {
    renderSearchResults(window.shops);
    updateSearchResults();
